            ...
            for elem in items:
                ...
                if self.elementXML.__contains__(type):
                    ...
                    try:
                        ...
                        self.ui.textEdit.append('Копирование ' + file + ' в ' + self.inBANK + ' успешно завершено')
                    except Exception:
                        self.ui.textEdit.append('Копирование ' + file + ' в ' + self.inBANK + ' не удалось')
                    ...
                    for bankfile in bankfiles:
                        if bankfile.__contains__(file):
                            ...
                            try:
                               os.remove(myFile)
                               self.ui.textEdit.append('Файл ' + file + ' удален из ' + self.isASFK)
                            except:
                                self.ui.textEdit.append('Не удалось удалить ' + file + ' из ' + self.isASFK)

        if isEmpty:
            sender = self.sender()
            self.ui.textEdit.append('Корневой каталог не содержит файлов ' + sender.text())
        ...
        for file in files:
            ...
            for elem in items:
                ...
                if self.elementXML.__contains__(self.OTVSEND):
                    print('OTVSEND ' + file)
                    self.ui.textEdit.setPlainText('OTVSEND ' + file)
