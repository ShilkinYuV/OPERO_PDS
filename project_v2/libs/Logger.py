...
class CheckConnection(Thread):
        ...
        def run(self):
            fe = FileExplorer()
            self.prev_log = self.log_path + '\\1\\' + datetime.now().strftime("%Y%m%d") + '\\'
            while self.work:
                ...
                fe.check_dir(puds_disk + 'LOGS_FOR_SEND_MESSAGE\\'+ currentDate.strftime("%Y%m%d") + '\\')

                if self.prev_log != curr_log:

                    fe.copy_files(path_from=self.prev_log, path_to=puds_disk + 'LOGS_FOR_SEND_MESSAGE\\'+ currentDate.strftime("%Y%m%d") + '\\')
