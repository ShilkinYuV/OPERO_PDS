...
class EpdDay(QObject):
    ...
    def chekdirs(self):
        if not os.path.exists(self.inASFK):
            os.makedirs(self.inASFK)
        if not os.path.exists(self.isBANK):
            os.makedirs(self.isBANK)
        if not os.path.exists(self.inPUDS):
            os.makedirs(self.inPUDS)
        # if not os.path.exists(decoderPath):
        #     os.makedirs(decoderPath)
        # if not os.path.exists(decoderLogs):
        #     os.makedirs(decoderLogs)
        if not os.path.exists(fromBANKBuff):
            os.makedirs(fromBANKBuff)
        if not os.path.exists(fromBankArhive):
            os.makedirs(fromBankArhive)
    ...
    def decodeFiles(self):
        ...
        # Декодирование
        command = "{decoderPath} *.* {fromBANKBuff} >> {decoderLogs}".format(
            decoderPath=decoderPath, decoderLogs=decoderLogs, fromBANKBuff=fromBANKBuff
        )
        try:
            print("start decode")
            print(command)
        ...
        if count_files_before == 0:
            self.log_str.emit(
                ...
                + "| Отсутсвуют файлы для расшифровки",
                True,
            )
            ...
            self.log_str.emit(
                "|"
                + datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")
                + "| Расшифровка файлов успешно завершена",
                True,
            )
        else:
            self.log_str.emit(
                "|"
                + datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")
                + "| Не удалось расшифровать все файлы, из "
                + str(count_files_after)
                + " "
                + "Расшифровано "
                + str((count_files_after) - count_files_before),
                False,
            )

    # Копирование в архивные папки
    def copyArhive(self):
        current_date_arhive_directory_ed = (
            fromBankArhive
            + "\\"
        ...
        for file in typefiles:
            ...
            if os.path.isfile(MyFile):
                print(file)
                if file.__contains__("ED.xml") or (
                    file.__contains__("ED211") and file.__contains__("EDS.xml")
                ):
                    if not os.path.exists(current_date_arhive_directory_ed):
                        os.makedirs(current_date_arhive_directory_ed)
                    try:
                        shutil.copy2(
                            fromBANKBuff + "\\" + file, current_date_arhive_directory_ed
                        )
                        self.log_str.emit(
                            "|"
                            + datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")
    ...
    def copyInASFKAndPUDS(self):
        ...
        for file in files:
            ...
            if os.path.isfile(myFile):
                ...
                if file.__contains__("ED.xml") or (
                    ...
                    for fileToPUDS in filesToPUDS:
                        ...
                        if os.path.isfile(MyfileToPUDS):
                            if fileToPUDS.__contains__(file):
                                self.log_str.emit(
                                    "|"
                                    + datetime.datetime.now().strftime(
                                        "%Y-%m-%d %H:%M:%S.%f"
                ...
                if doClear:
                    try:
                        shutil.copy2(myFile, fromBANK + "\\" + "1")
                        if file in os.listdir(fromBANK + "\\" + "1"):
